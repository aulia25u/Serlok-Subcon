...
@section('title', 'LOI - Create Team Feasibility Commitment')
@section('page-title', 'LOI - Create Team Feasibility Commitment')
                        ...
                        <form id="feasibilityForm">
                                ...
                                <div class="row mt-3">
                                    <div class="col-md-6">
                                        ...
                                        <div class="row mb-2">
                                            ...
                                            <div class="col-sm-8">
                                                <select class="form-control form-control-sm" id="part_select"
                                                    name="part_select" required>
                                        ...
                                        <div class="row mb-2">
                                            <div class="col-sm-8">
                                                <input hidden type="text" class="form-control form-control-sm"
                                                    id="customer_id" name="customer_id" readonly>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="conclusion-section">
                                ...
                                <div class="mt-3">
                                    ...
                                    <textarea class="form-control" name="conclusion_notes" rows="3"></textarea>
                                </div>
                            </div>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <input type="date" class="form-control form-control-sm"
                                                    name="revisions[0][revision_date]" value="2025-04-08" required>
                                            </td>
                                            <td>
                                                <input type="text" class="form-control form-control-sm"
                                                    name="revisions[0][revision_contains]" value="Perubahan point check"
                                                    required>
                                            </td>
...
@push('js')
    <script>
        $(document).ready(function() {
            let revisionCount = 1; // Start from 1 since we have one default revision

            // Initialize Select2 for part selection
            $('#part_select').select2({
                theme: 'bootstrap4',
                ajax: {
                    url: '{{ route('loi.feasability-commitment.parts') }}',
                    dataType: 'json',
                    delay: 250,
            ...
            // When part is selected, auto-fill other fields
            $('#part_select').on('select2:select', function(e) {
                const data = e.params.data;

                // Set part number and name
                $('#part_no').val(data.partno);
                $('#part_name').val(data.partname);

                // Get additional details including customer and model
                $.ajax({
                    url: '{{ route('loi.feasability-commitment.part-details') }}',
                    type: 'GET',
                    data: {
                        part_id: data.id
                    },
                    success: function(response) {
                        if (response.success) {
                            ...
                            if (!response.data.model || response.data.model.trim() === '') {
                                Swal.fire({
                                    icon: 'warning',
                                    title: 'Model Not Found',
                                    text: 'Data model untuk part ini tidak ditemukan. Silakan periksa kembali atau hubungi administrator.',
                            ...
                            Swal.fire({
                                icon: 'error',
                                title: 'Error',
                                text: response.message || 'Failed to load part details.'
                            });
                        }
                    },
                    error: function(xhr) {
                        console.error('Error fetching part details:', xhr);

                        let errorMessage = 'Gagal mengambil detail part.';
                        ...
                        Swal.fire({
                            icon: 'error',
                            title: 'Error Loading Part Details',
                            text: errorMessage,
            ...
            $('#feasibilityForm').on('submit', function(e) {
                ...
                // Validate customer_id is filled
                if (!$('#customer_id').val() || $('#customer_id').val().trim() === '') {
                    Swal.fire({
                        icon: 'warning',
                        title: 'Customer ID Required',
                        text: 'Please select a part to auto-fill the Customer ID.',
                        confirmButtonText: 'OK'
                    });
                    return false;
                }
                ...
                $.ajax({
                    url: '{{ route('loi.feasability-commitment.store') }}',
                    type: 'POST',
                    data: formData,
                    success: function(response) {
                        Swal.fire({
