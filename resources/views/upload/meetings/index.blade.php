...
@section('title', 'Upload Hasil Meeting')
@section('page-title', 'Upload Hasil Meeting')
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Upload Hasil Meeting</h3>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="addModalLabel">Add New Hasil Meeting</h5>
...
<div class="modal fade" id="viewModal" tabindex="-1" role="dialog" aria-labelledby="viewModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="viewModalLabel">View Hasil Meeting</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
...
<script>
    $(document).ready(function() {
        ...
        $(document).on('click', '.add-drawing-btn', function() {
            ...
            $('#addModalLabel').text('Add New Hasil Meeting');
            $('#partId').val(partId);
            $('#addModal').modal('show');
        });
        ...
        $('#attachmentForm').on('submit', function(e) {
            ...
            $.ajax({
                url: "{{ route('upload.meeting.store') }}",
                type: 'POST',
                data: formData,
                processData: false,
                contentType: false,
                success: function(response) {
                    ...
                    $('#successDescription').text('Hasil Meeting telah berhasil disimpan ke dalam sistem.');
        ...
        $(document).on('click', '.view-btn', function() {
            let partId = $(this).data('id');
            let viewUrl = "{{ url('upload/meeting') }}/" + partId;
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.length > 0) {
                        ...
                        drawingListHtml = '<div class="col-12"><p class="text-center">No Hasil Meeting found for this part.</p></div>';
                    }
        ...
        $(document).on('click', '.delete-drawing-btn', function() {
            ...
            let deleteUrl = `/upload/meeting/${drawingId}`;
            let button = $(this);

            if (confirm('Apakah anda yakin akan menghapus hasil meeting?')) {
                $.ajax({
                    url: deleteUrl,
                    type: 'DELETE',
                    success: function(response) {
                        $('#successMessage').text(response.success);
                        $('#successDescription').text('Hasil Meeting telah berhasil dihapus dari sistem.');
                        $('#successModal').modal('show');
                        button.closest('.col-md-4').remove();
                        if ($('#drawingList .col-md-4').length === 0) {
                            $('#drawingList').html('<div class="col-12"><p class="text-center">No Hasil Meeting found for this part.</p></div>');
                        }
                    },
                    error: function(xhr) {
